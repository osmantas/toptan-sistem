                    ...
                    html += `<tr>
                        ...
                        <td>
                            <div style="display:flex; align-items:center; gap:8px;">
                                ...
                                <span style="font-size:0.8rem; color:${yuzde >= 100 ? 'var(--success)' : 'var(--accent)'}; font-weight:600; white-space:nowrap;">${teslimEdilen.toFixed(1)} KG (${yuzde}%)</span>
                            </div>
                        </td>
                        ...
                        <td>
                            ...
                            <button class="btn-sm" style="background:#6610f2; white-space:nowrap;" onclick="document.getElementById('teslim-miktar-${d.detay_id}').value=${kalan.toFixed(1)}" title="Tamamını teslim et">📦 Tamamı</button>
                        </td>
                    </tr>`;
                });
                html += '</tbody></table>';
        ...
        function renderTeslimatFisiAccordion(data, container) {
            ...
            data.forEach((fis, idx) => {
                const  tarihDate = new D ate(fis.olusturm a_tarihi);
                 const t arihStr = fis.olusturma_tarihi ? tarihDate.toLocaleDateString('tr-TR', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : '-';
                ...
                html += `
                    <div class="product-accordion" id="tfacc-${idx}">
                        <div class="accordion-header" onclick="toggleTeslimatFisiAccordion(${idx}, ${fis.fis_id})">
                            <span class="product-code" style="min-width:140px;">🧾 Fiş #${fis.fis_id}</span>
                            ...
                            <div class="product-stats">
                                ...
                                <button class="btn-sm btn-teslim" style="margin-left:10px;" onclick="event.stopPropagation(); printTeslimatFisiProfesyonel(${fis.fis_id})">🖨️ Fiş Yazdır</button>
                            </div>
                            <span class="accordion-chevron">▼</span>
                        </div>
        ...
        async function toggleTeslimatFisiAccordion(idx, fisId) {
            ...
            try {
                ...
                let html = `<table>
                    <thead><tr>
                        <th>#</th><th>Ürün Kodu</th><th>Ürün Adı</th><th>Miktar (KG)</th><th>Birim Fiyat (USD)</th><th>İskonto (%)</th><th>Net Tutar (USD)</th>
                    </tr></thead>
                    <tbody>`;
                ...
                data.forEach((item, i) => {
                    ...
                    const net = brut - (brut * iskonto / 100);
                    brutToplam += brut;
                    netToplam += net;
        ...
        async function printTeslimatFisiProfesyonel(fisId) {
            ...
            try {
                ...
                const tarihStr = tarihDate.toLocaleDateString('tr-TR', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' });
                ...
                const printHTML = `<html><head><title>Teslimat Fişi - ${firmaAdi} (#${fisId})</title>
                <style>
                ...
                <div class="fis-header">
                    <h1>AKSA TOPTAN</h1>
                    <div class="subtitle">Teslimat Fişi (Arşiv)</div>
                </div>
                <div class="fis-info">
                    <div class="col">
                    ...
                    <div class="col" style="text-align:right;">
                        <strong>Oluşturma Tarihi:</strong> ${tarihStr}<br>
                        <strong>Sayfa:</strong> 1/1<br>
                    </div>
                </div>
                <table>
                    <thead><tr>
                        ...
                        <th style="width:110px; text-align:right;">Net Tutar</th>
                    </tr></thead>
                    <tbody>${satirlar}</tbody>
                </table>
                <div class="totals-box">
                    ...
                    <div class="totals-row grand">
                        <div class="label">NET TUTAR:</div>
                        <div class="value">$ ${genelNet.toFixed(2)}</div>
                    </div>
                    ${currentUsdRate > 0 ? `
                        <div class="totals-row grand" style="background:#28a745; border-top:1px solid #1e7e34;">
                            <div class="label" style="color:#e9ecef;">TL KARŞILIĞI:</div>
                            <div class="value">₺ ${genelTL.toFixed(2)}</div>
                        </div>
                    ` : ''}
                </div>
                ${currentUsdRate > 0 ? `<div class="kur-info">* İşlem tarihindeki veya güncel kur baz alınmıştır (1 USD = ${currentUsdRate.toFixed(2)} ₺)</div>` : ''}
                <div class="sign-area">
                    <div class="sign-box">
                        <div class="line">Teslim Eden</div>
                    </div>
                ...
                <div class="footer">
                    Bu belge AKSA TOPTAN sistemi üzerinden elektronik olarak üretilmiştir.<br>
                    www.aksatoptan.com
                </div>
                </body></html>`;
        ...
        function loadRaporlar() {
            const html = `
                <div class="card">
                    <div class="card-title">Genel Satış ve Teslimat Raporları</div>
                    <div class="filter-bar" style="justify-content: space-between; flex-wrap: wrap;">
                        <div style="display:flex; gap:10px; align-items:center;">
                            <label>Tarih Seçimi:</label>
                            <input type="date" id="raporTarihBas" class="filter-select">
                            <span>-</span>
                            <input type="date" id="raporTarihSon" class="filter-select">
                        </div>
                        <div style="display:flex; gap:10px; align-items:center; margin-top:10px;">
                            <label>Müşteri:</label>
                            <select id="raporMusteri" class="filter-select">
                                <option value="">Tümü</option>
                            </select>
                            <button class="btn-sm btn-bulk-primary" onclick="fetchRaporData()">Filtrele / Getir</button>
                        </div>
                    </div>
                    
                    <div style="display:flex; gap:20px; margin-bottom:20px; flex-wrap:wrap;">
                        <div class="card" style="flex:1; min-width:200px; border-color:var(--accent); background:rgba(54,153,255,0.05); padding:15px; margin-bottom:0;">
                            <h3 style="font-size:1rem; color:var(--text-light); margin-bottom:10px;">Toplam Ciro (Brüt)</h3>
                            <div id="raporCiroBrut" style="font-size:1.4rem; font-weight:bold; color:var(--accent);">$ 0.00</div>
                        </div>
                        <div class="card" style="flex:1; min-width:200px; border-color:var(--danger); background:rgba(246,78,96,0.05); padding:15px; margin-bottom:0;">
                            <h3 style="font-size:1rem; color:var(--text-light); margin-bottom:10px;">Toplam İskonto</h3>
                            <div id="raporIskonto" style="font-size:1.4rem; font-weight:bold; color:var(--danger);">- $ 0.00</div>
                        </div>
                        <div class="card" style="flex:1; min-width:200px; border-color:var(--success); background:rgba(27,197,189,0.05); padding:15px; margin-bottom:0;">
                            <h3 style="font-size:1rem; color:var(--text-light); margin-bottom:10px;">Net Toplam (USD)</h3>
                            <div id="raporCiroNet" style="font-size:1.4rem; font-weight:bold; color:var(--success);">$ 0.00</div>
                        </div>
                        <div class="card" style="flex:1; min-width:200px; border-color:#ffc107; background:rgba(255,193,7,0.05); padding:15px; margin-bottom:0;">
                            <h3 style="font-size:1rem; color:var(--text-light); margin-bottom:10px;">Tahmini Karşılık (TL)</h3>
                            <div id="raporCiroTL" style="font-size:1.4rem; font-weight:bold; color:#ffc107;">₺ 0.00</div>
                        </div>
                    </div>

                    <div style="margin-top:20px;">
                        <h3 style="margin-bottom:15px; font-size:1.1rem; color:var(--text-light);">Teslim Edilen Ürünler (Özet)</h3>
                        <table>
                            <thead>
                                <tr>
                                    <th>Ürün Kodu</th>
                            ...
                            <tbody id="raporUrunlerBody">
                                <tr><td colspan="6" style="text-align:center;">Lütfen filtreleme yapın.</td></tr>
                            </tbody>
                        </table>
                    </div>
                </div>
            ...
            fetch('../api/siparis_api.php?action=musteriler_listesi')
                .then(r => r.json())
                .then(data => {
                    const sel = document.getElementById('raporMusteri');
        ...
        async function fetchRaporData() {
            const tbody = document.getElementById('raporUrunlerBody');
            const btn = document.querySelector('button[onclick="fetchRaporData()"]');
            if (btn) btn.innerHTML = 'Yükleniyor...';
            
            try {
                let url = '../api/siparis_api.php?action=raporlar';
                const tb = document.getElementById('raporTarihBas')?.value;
                const ts = document.getElementById('raporTarihSon')?.value;
                const mid = document.getElementById('raporMusteri')?.value;
                ...
                if (!data || data.length === 0) {
                    html = '<tr><td colspan="6" style="text-align:center;">Bu kriterlere uygun kayıt bulunamadı.</td></tr>';
                } else {
                    data.forEach(u => {
                        const miktar = parseFloat(u.toplam_kg || 0);
                ...
                const iskontoTop = toplamBrut - toplamNet;
                const tlKarsilik = currentUsdRate > 0 ? toplamNet * currentUsdRate : 0;

                document.getElementById('raporCiroBrut').textContent = '$ ' + toplamBrut.toFixed(2);
                document.getElementById('raporIskonto').textContent = '- $ ' + iskontoTop.toFixed(2);
                document.getElementById('raporCiroNet').textContent = '$ ' + toplamNet.toFixed(2);
                document.getElementById('raporCiroTL').textContent = '₺ ' + tlKarsilik.toFixed(2);

            } catch (err) {
                console.error(err);
                tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;color:var(--danger);">Hata oluştu.</td></tr>';
            }
            if (btn) btn.innerHTML = 'Filtrele / Getir';
        }
