...
if ($method === 'GET') {
    ...
    if (empty($id) || empty($urun_kodu) || empty($urun_adi) || !is_numeric($usd_fiyat)) {
        http_response_code(400);
        echo json_encode(['status' => 'error', 'message' => 'Lütfen tüm bilgileri doğru formatta girin.']);
        exit;
    }
    ...
    try {
        $stmt = $pdo->prepare("UPDATE urunler SET urun_kodu = :urun_kodu, urun_adi = :urun_adi, usd_fiyat = :usd_fiyat WHERE id = :id");
        $stmt->execute(['urun_kodu' => $urun_kodu, 'urun_adi' => $urun_adi, 'usd_fiyat' => $usd_fiyat, 'id' => $id]);
        echo json_encode(['status' => 'success', 'message' => 'Ürün başarıyla güncellendi.']);
    } catch (PDOException $e) {
        if ($e->errorInfo[1] == 1062) {
            http_response_code(400);
            echo json_encode(['status' => 'error', 'message' => 'Bu ürün kodu zaten başka bir üründe kullanılıyor.']);
        } else {
            http_response_code(500);
