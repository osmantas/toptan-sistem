...
if ($method === 'GET') {
    // Ürünleri listele (fiyat OLMADAN - müşteri için)
    try {
        $page = isset($_GET['page']) ? max(1, intval($_GET['page'])) : 1;
        $limit = isset($_GET['limit']) ? max(1, intval($_GET['limit'])) : 5000;
        $offset = ($page - 1) * $limit;
        ...
        // Verileri getir
        $stmt = $pdo->prepare("SELECT id, urun_kodu, urun_adi FROM urunler $whereSQL ORDER BY urun_kodu ASC LIMIT ? OFFSET ?");
    ...
    // Yeni ürün ekle (buraya fiyat geliyor ama admin üzerinden)
    ...
    try {
        ...
        if ($e->errorInfo[1] == 1062) {
            http_response_code(400);
            echo json_encode(['status' => 'error', 'message' => 'Bu ürün kodu zaten mevcut. Lütfen benzersiz bir kod kullanın.']);
        } else {
            http_response_code(500);
            echo json_encode(['status' => 'error', 'message' => 'Kayıt sırasında bilinmeyen hata oluştu.']);
        }
    }
} else {
